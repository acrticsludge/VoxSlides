...
**Root cause:** AI-generated design output (from Stitch/Figma) invented custom utility names that look plausible but aren't real Tailwind classes.

**Fix:** Replaced all `unit-*` classes with standard Tailwind (`px-8`, `py-8`, `mx-8`), `font-label-mono` with `font-mono`, and `text-code-sm` with `text-xs`.

**Prevention:** After any AI-generated design, grep for non-standard class patterns (`-unit-`, `text-code-`, `font-label-`) and verify they exist in the Tailwind docs. Tailwind's silent failure on unknown classes makes this invisible until you inspect the rendered output.

## 2026-05-31: [Bug] Dark-mode color tokens used on light theme made chip text invisible

**What happened:** `CONDITION_COLORS` in `conditions.ts` used `text-amber-300`, `text-blue-300` etc. — these are light-colored text meant for dark backgrounds. On the light theme (`#fbf9f7` background), the chip text was nearly invisible.

**Root cause:** Color tokens were likely copied from a dark-mode design system or generated by AI without checking the active theme.

**Fix:** Switched to `bg-{color}-100 text-{color}-800 border-{color}-300` — light backgrounds with dark text, readable on the light theme.

**Prevention:** When using color utilities, always check: "Is this text readable on my actual background?" Light themes need `-800`/`-900` text, dark themes need `-200`/`-300` text.

## 2026-05-31: [Bug] AI-generated design output (Figma via Stitch) must be audited against actual codebase before implementation
